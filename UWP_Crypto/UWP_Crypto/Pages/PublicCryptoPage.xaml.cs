...
/// </summary>
public sealed partial class PublicCryptoPage : Page
{
    private PublicKeyItem SelectedKey;

    public PublicCryptoPage()
    {
        InitializeComponent();
    }
    ...
    {
        if (RunType.SelectedIndex == 1 && SelectedKey != null && !SelectedKey.Holder.HasPrivate)
        {
    ...
    private async void Transform_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (RunType.SelectedIndex == 1 && SelectedKey.Holder.IsEncrypted)
        {
        ...
        await Task.Run(() => RunCrypto(runType, outType, input, password));

        ProgressAnim.IsActive = false;
        ChangeEnability(true);
    }

    private async void RunCrypto(int runType, int outType, string input, string password)
    {
        try
        {
            ...
            if (outType == 0)
            {
                if (runType == 0)
                {
                    result = await RSAHelper.Encrypt(SelectedKey.Name, input, RSAHelper.OutEncoding.Base64);
                }
                else
                {
                    result = await RSAHelper.Decrypt(SelectedKey.Name, input, RSAHelper.OutEncoding.Base64, password);
                }
            }
            else
            {
                if (runType == 0)
                {
                    result = await RSAHelper.Encrypt(SelectedKey.Name, input, RSAHelper.OutEncoding.Hex);
                }
                else
                {
                    result = await RSAHelper.Decrypt(SelectedKey.Name, input, RSAHelper.OutEncoding.Hex, password);
                }
            }
